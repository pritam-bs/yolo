...
- **Dynamic Inference Throttling:** The application intelligently monitors system metrics (GPU usage, temperature, battery level) and adjusts YOLO inference parameters in real-time. This ensures optimal performance without overloading the device, preventing thermal throttling and excessive battery drain.
...
- **Real-time UI Updates:** The UI displays the device's current system health status as `NORMAL`, `THROTTLED`, or `CRITICAL`, providing instant feedback.
...
The custom `system_monitor` package provides a stream of `SystemHealthState` data. The application listens to this stream and dynamically adjusts the confidence and IoU (Intersection over Union) thresholds for the YOLO model. For example, if the system state becomes `THROTTLED`, the application might increase the confidence threshold to reduce the number of detections, thereby lowering the computational load.
